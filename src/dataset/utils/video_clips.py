...
class VideoClips(object):
    ...
    def _init_from_metadata(self, metadata):
        ...
        self.video_dir = metadata["video_dir"]
    ...
    def get_clip(self, idx):
        ...
        if backend == "pyav":
            ...
            #print(clip_pts, len(clip_pts), video_path)
            #print(self.video_dir+'/'+video_path)
            video, audio, info = read_video(self.video_dir+'/'+video_path, start_pts, end_pts)
        else:
        ...
        assert len(video) == self.num_frames, "{} x {}, {}. {}, {}, {}, {},{}".format(
            video.shape, self.num_frames, clip_pts, len(clip_pts), start_pts, end_pts, video_path,'!'*10
        )
        return video, audio, info, video_idx, clip_pts
